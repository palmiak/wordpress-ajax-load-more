   	...
   	public function alm_query_posts() {
         ...
         $paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
   		$args = array(
   			...
   			'paged'                    => $paged,
			'page'					   => $paged, // SearchWP needs page instead of paged
   		);
		...
		// only use SWP query if SWP is installed AND $args[s] is not empty
	   	if ( class_exists( 'SWP_Query' ) && isset( $args['s'] ) && '' != $args['s'] ) {
			$alm_query = new SWP_Query( $args ); // SWP_Query() for searchWP
		} else {
         ...
         if($queryType === 'standard'){
	   		...
	   		// Run the loop
			if ( ! empty( $alm_query->posts ) ) {
				//without next line first post in query can go missing
				global $post;
               ...
               $cta_array = Array();
	            ...
	            // ALM Loop - modified for Search WP - foreach instead of while
				foreach ( $alm_query->posts as $post ) :
					setup_postdata( $post );
	   				$alm_loop_count++;
	   				$alm_current++;
